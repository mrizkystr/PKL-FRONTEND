...
const SalesTargetTracking = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      salesApi.dataPsApi.getTargetTrackingAndSalesChart(
        currentMonth,
        currentYear
      ),
  });
  ...
  const saveTargetMutation = useMutation({
    mutationFn: (formData) => salesApi.dataPsApi.saveTargetGrowth(formData),
    onSuccess: () => {
  ...
  return (
    ...
    >
      <Box
        sx={{ padding: 3, maxWidth: 1200, margin: "0 auto", marginTop: "50px" }}
      >
        {/* Form Set Target */}
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={3}>
              <Typography variant="body1">
                ...
                {formatPercentage(data?.performance_data?.daily_target_average)}
              </Typography>
            </Grid>
            <Grid item xs={3}>
              <Typography variant="body1">
                MTD Realization:{" "}
                {formatNumber(data?.performance_data?.mtd_realization)}
              </Typography>
            </Grid>
            <Grid item xs={3}>
              <Typography variant="body1">
                Achievement Target Growth:{" "}
                {formatPercentage(
                  data?.performance_data?.achievement_target_growth
                )}
              </Typography>
            </Grid>
            <Grid item xs={3}>
              <Typography variant="body1">
                Achievement Target RKAP:{" "}
                {formatPercentage(
                  data?.performance_data?.achievement_target_rkap
                )}
              </Typography>
            </Grid>
          </Grid>
